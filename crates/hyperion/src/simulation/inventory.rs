...
use hyperion_inventory::{ CursorItem, Inventory, InventoryState, ItemSlot, OpenInventory, PlayerInventory };
use hyperion_utils::EntityExt;
use tracing::debug;
use valence_protocol::{ packets::play::{ self, ClickSlotC2s, UpdateSelectedSlotC2s }, VarInt, Decode };
                ...
                if changed {
                    // group the player's inventory and the open inventory together
                    {
                        let inventories_mut = if let Some(open_inv) = open_inv {
                            let slots = inventory.slots_mut();
                            slots.append(open_inv.slots_mut());
                            slots
                        } else {
                            inventory.slots_mut()
                        };
...
pub fn close_handled_screen(
    ...
    debug!("close handled screen: {}", pkt.window_id);
